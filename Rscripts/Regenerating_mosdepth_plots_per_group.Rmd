...
bedwindows = read.table("mosdepth.10000bp.gg.tab.gz",header=F)
...
d$Strain = factor(d$Strain, levels = c("Ex1", "Ex2", "Ex3", "Ex4", "Ex5", "Ex6", "Ex7", "Ex8"))
...
p1 <- ggplot(d,aes(x=pos,y=Depth, color=Strain, fill=Strain)) +
  ...
  labs(title="Genomewide Coverage in 10kb Windows Across all 23 strains of Exophiala
  dermatitidis",xlab="Position",y="Normalized Read Depth") +
  ...
  theme_classic() + guides(color = guide_legend(override.aes = list(size = 2.5)))
#+ guides(color=FALSE) 
#
...
ggsave("Genomewide_cov_by_10kb_win_mosdepth_1-8.png",p1, width=10, height=4)
ggsave("Genomewide_cov_by_10kb_win_mosdepth_1-8.pdf",p1, width=10, height=4)
```
...
```{R}
#Run for 5kb windows
bedwindows5kb = read.table("mosdepth.v2.5000bp.gg.tab.gz",header=F)
